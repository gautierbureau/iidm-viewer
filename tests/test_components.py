...
def test_vl_selector_populates_session_state(xiidm_upload):
    ...
    assert at.session_state["selected_vl"] in at.selectbox(key="vl_selectbox").options


def test_vl_filter_no_match_renders_info(xiidm_upload):
    at = _prepare(xiidm_upload)
    at.text_input(key="vl_filter_text").set_value("ZZZZZZZ").run(timeout=30)
...
def test_vl_filter_narrows_selectbox_options(xiidm_upload):
    at = _prepare(xiidm_upload)
    at.text_input(key="vl_filter_text").set_value("VL1").run(timeout=30)
    assert not at.exception
    options = at.selectbox(key="vl_selectbox").options
