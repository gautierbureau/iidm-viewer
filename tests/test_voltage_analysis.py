...
def test_bus_voltages_voltage_level_ids_are_strings(node_breaker_network):
    df = _bus_voltages(node_breaker_network)
    assert df["voltage_level_id"].dtype == object


# ── _shunt_compensation — empty network ────────────────────────────────────────

def test_shunt_compensation_empty_when_no_shunts(node_breaker_network):
    df = _shunt_compensation(node_breaker_network)
    assert df.empty
...
def test_shunt_compensation_current_q_estimated_without_lf(network_with_shunt):
    ...
    # No load flow has run → q should be estimated as b_per_section × section_count × V²
    expected = _SHUNT_B_PER_SECTION * _SHUNT_SECTION_COUNT * row["nominal_v"] ** 2
...
def test_shunt_compensation_available_q_covers_remaining_sections(network_with_shunt):
    ...
    expected = _SHUNT_B_PER_SECTION * remaining * row["nominal_v"] ** 2
...
def test_shunt_compensation_total_q_is_max_capacity(network_with_shunt):
    df = _shunt_compensation(network_with_shunt)
    row = df[df["id"] == _SHUNT_ID].iloc[0]
    expected = _SHUNT_B_PER_SECTION * _SHUNT_MAX_SECTION_COUNT * row["nominal_v"] ** 2
...
def test_svc_q_range_matches_b_min_b_max(node_breaker_network):
    ...
    svcs = node_breaker_network.get_static_var_compensators(
        attributes=["b_min", "b_max", "voltage_level_id"]
    )
    svc = svcs.loc[row["id"]]
...
def test_svc_current_q_nan_without_lf(node_breaker_network):
    df = _svc_compensation(node_breaker_network)
    # four_substations has no solved load flow → current_q_mvar should be NaN
    assert df["current_q_mvar"].isna().all()
